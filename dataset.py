...
class FakeNewsDataset(Dataset):
    ...
    def __init__(self,train=True):
        self.train=train
        if self.train == True:
            self.data = pd.read_csv(config.TRAIN_DATA_PATH,usecols=config.FEATURES).values
            self.labels =  pd.read_csv(config.TRAIN_DATA_PATH,usecols=config.TARGET).values
        else:
            self.data = pd.read_csv(config.TEST_DATA_PATH,usecols=config.FEATURES).values
